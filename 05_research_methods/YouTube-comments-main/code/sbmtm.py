...
class sbmtm():
    ...
    def make_graph_new(self, df_user):
        '''
        ...
        videos_list = df_user['VideoID'].unique()
        D = len(videos_list)
        ...
        ## name: videos - VideoID, users - AuthorID
        ## kind: videos - 0, users - 1
        ...
        for i_d in range(D):
            video = videos_list[i_d]
            v = videos_add[video]


        ## add all videos and users as nodes
        ## add all activity as links
        for i_d in tqdm(range(D)):
            video = videos_list[i_d]
            users = df_user[df_user['VideoID'] == video]['AuthorID']
